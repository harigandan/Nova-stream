...
export default function CategoryPage() {
  ...
  const categoryName = (params.name as string).replace('-', ' ');
  ...
  useEffect(() => {
      ...
      const loadCategoryData = async () => {
        setLoading(true);
        try {
            const apiCategory = categoryName.toLowerCase() === 'ipl' ? 'ipl' : categoryName;
            const [slidesData, liveMatchesData, upcomingMatchesData, highlightsData] = await Promise.all([
                fetchSlides(apiCategory),
                fetchLiveMatches(apiCategory),
                fetchUpcomingMatches(apiCategory),
                fetchHighlights(apiCategory)
            ]);
  ...
  return (
    <div className="bg-background text-white min-h-screen flex flex-col">
        ...
        <div className="flex items-center justify-between px-10 py-2">
          ...
          >
            ...
            <div>
              ...
              <p className="text-sm text-gray-400 capitalize">{decodeURIComponent(categoryName)}</p>
            </div>
          </motion.div>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar">
        ...
        ) : <div className="text-center py-10">No featured content for {decodeURIComponent(categoryName)} right now.</div>}
        ...
        <main className="px-24 py-6">
          ...
          {categoryName.toLowerCase() === 'football' && (
              <section className="mt-10">
                <h3 className="text-xl font-bold mb-4">Live Matches</h3>
                <SportsWidget type="games" sport="football" />
                <h3 className="text-xl font-bold mt-8 mb-4">League Standings</h3>
                <SportsWidget type="scoreboard" sport="football" options={{ "league": 39, "season": 2023 }} />
              </section>
          )}

          {categoryName.toLowerCase() === 'cricket' && (
              <section className="mt-10">
                <SportsWidget type="games" sport="cricket" />
              </section>
          )}

          {categoryName.toLowerCase() === 'ipl' && (
              <section className="mt-10">
                <h3 className="text-xl font-bold mb-4">Live IPL Matches</h3>
                <SportsWidget type="games" sport="cricket" options={{ "league_id": "4333" }} />
                <h3 className="text-xl font-bold mt-8 mb-4">IPL Standings</h3>
                <SportsWidget type="scoreboard" sport="cricket" options={{ "league_id": "4333" }} />
              </section>
          )}
          ...
          {loading ? <Skeleton className="h-48 w-full mt-10" /> : filteredHighlights.length > 0 && (
          <section className="mt-10">
           ...
           {!loading && categoryName.toLowerCase() !== 'football' && categoryName.toLowerCase() !== 'cricket' && categoryName.toLowerCase() !== 'ipl' && filteredLiveMatches.length === 0 && filteredUpcomingMatches.length === 0 && filteredHighlights.length === 0 && (
             <div className="text-center py-20 text-gray-500">
                <p>No matches or highlights available for {decodeURIComponent(categoryName)} at the moment.</p>
